...
Список загружается напрямую из API МойСклад (опрос каждые 5 секунд). Заказы, оформленные на витрине, помечаются комментарием `Заказ с сервиса Литком НН` и бейджем «С этого сервиса».
